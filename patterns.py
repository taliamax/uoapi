...
prereq = {
	...
	"prereqs" : re.compile(r"(?<=^(?:/ )?(?:Prerequisite|Préalable)s?\s*:\s*(?:One of )?)%s$" % parsable_codes),
	"coreqs" : re.compile(r"(?:Corequisite|Concomitant)\s*:\s*%s|%s(?= (?:is|are)(?: prerequisite or)? corequisite to %s$)|(?<=Les? cours )%s(?= (?:sont|est)(?: préalables ou)? concomitants? à %s$)" % (parsable_codes, parsable_codes, course_code_pattern, parsable_codes, course_code_pattern)),
